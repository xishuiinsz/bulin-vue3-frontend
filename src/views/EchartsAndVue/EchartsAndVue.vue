    ...
    <div class="container">
      ...
      <div>
        <div>
          来自百度的
          <a
            href="https://echarts.apache.org/examples/zh/editor.html?c=line-marker"
            >折线图示例</a
          >
          <div
            style="width: 1200px; height: 600px; margin-top: 16px"
            id="line-bar"
          ></div>
        </div>
      </div>
...
const myTooltipFormatter = async (params) => {
  const comp = (await import('./MyTooltip.vue')).default
  const htmlTemplate = await renderToString(createApp(comp, { list: params }))
  return htmlTemplate
}
